...
$dataresult = $DB->query('SELECT utctime, kwh FROM readings WHERE meterid="' . $DB->escapeString($_GET['meterid']) . '" AND strftime("%Y", utctime, "unixepoch") = "' . $DB->escapeString($_GET['value']) . '" GROUP BY strftime("%H %d %m %Y", utctime, "unixepoch")  ORDER BY utctime ASC ');
